...
def verify_fsdn(network, station, starttime=STARTTIME, component_code='HN*'):
    """
    ...
    :return: Raises the relevant Client exception if not available, otherwise returns True
    """
    client = Client(CLIENT_NAME)
    time = starttime

    # Raises Exception if not able to get waveforms 
    assert client.get_waveforms(network.code, station.code, "*", component_code, time, time + DURATION)
    return True
